...
cartList.forEach((value)=>{
    ...
    let stTotal=(value.quantity*value.price).toString().replace(/\B(?=(\d{3})+(?!\d))/g,',')
    trList+=`<tr>`
    trList+=`<td>`
    trList+=`<img src="../img/json/${value.photo}"alt="이미지네임"></td>`
    trList+=`<td class="pricename">${value.name}<br><br><span class="price won">${value.price}</span></td>`
    ...
    trList+=`<td class="myTotal won">${(value.quantity*value.price)}</td>`
    trList+=`<td>무료배송</td>`
    trList+=`<td>
    ...
    trList+=`</tbody>
            <tfoot>
            <tr>
            <td colspan="5">합계 :<span class="won">${total.toString().replace(/\B(?=(\d{3})+(?!\d))/g,',')}</span>
            </td>
            </tr>
            </tfoot>
            </table>`
